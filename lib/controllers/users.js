...
exports.addWorkouts = function(req, res, next) {
    ...
    User.findById(req.user.id, function(err, user) {
        var formattedDate = moment(req.body.date).format('L');

        // Look for existing workout with date
        var workoutIdx = _.findIndex(user.workouts, {
            date: formattedDate
        });

        /**
         * If existing date is found merge in lifts and append comments
         */
        if (workoutIdx >= 0) {
            var existingWorkout = user.workouts[workoutIdx];

            existingWorkout.lifts = existingWorkout.lifts.concat(req.body.lifts);

            if (existingWorkout.comments) {
                existingWorkout.comments = existingWorkout.comments.concat('\n' + req.body.comments);
            } else {
                existingWorkout.comments = req.body.comments;
            }
